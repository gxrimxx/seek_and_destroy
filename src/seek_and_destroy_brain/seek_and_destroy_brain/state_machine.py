...
class StateMachine(Node):
    ...
    def on_set_target(self, msg: String):
        data = msg.data.strip()
        self.get_logger().info(f'GUI set_target: {data}')
        if data == 'QUIT':
        ...
        if data == 'SEARCH_AGAIN':
            ...
            return
        if self.state == 'IDLE':
            self._gui_target_pending = format_target(data)
            print(f'\n  [GUI] Target received: {self._gui_target_pending}')
    ...
    def on_detection(self, msg: String):
        ...
        if name == current_target:
            if conf >= self.CONF_CONFIRM:
                self._on_target_confirmed(name, map_x, map_y)
                return
            if accumulated >= self.CONF_INVESTIGATE and current_state == 'EXPLORING':
                rx, ry   = self._get_robot_pose()
                approach = self._approach_point(rx, ry, map_x, map_y, self.ROAM_STANDOFF_METRES)
                self._invest_name = name
                self._invest_pos  = (map_x, map_y)
                print(f'\n  [INVESTIGATE] Possible "{name}" (score={accumulated:.3f}) — driving closer...')
                self._set_explore(False)
                self._publish_cmd(f'INVESTIGATE:{approach[0]:.4f},{approach[1]:.4f}')
        ...
        if conf >= self.CONF_CONFIRM and name not in self._incidental_targets:
            ...
            print(f'\n  [INCIDENTAL] "{name}" at ({map_x:.2f},{map_y:.2f}) — not the target.')
