...
class ColorDetector(Node):
    ...
    def _callback_rgbd(self, color_msg: Image, depth_msg: Image):
        ...
        if det['name'] is None:
            self._publish_result(None, 0.0, 0.0, 0.0)   # ← publish empty instead of just debug
    ...
    def _depth_to_map(self, cx_px, cy_px, depth_frame, header):
        ...
        try:
            ...
            # Alternative: use lookup with explicit time tolerance
            pt_map = tf2_geometry_msgs.do_transform_point(
                pt,
                self.tf_buffer.lookup_transform(
                    'map',
                    'camera_color_optical_frame',
                    rclpy.time.Time(),           # use latest available, not message timestamp
                    timeout=rclpy.duration.Duration(seconds=0.3)
                )
            )
    ...
    def _publish_result(self, name: str, conf: float,
        ...
        if name and conf > 0.01:
            ...
            msg.data = 'none:0.0000:0.0000:0.0000'   # ← explicit clear signal
